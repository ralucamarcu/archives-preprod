...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[saisie_donnees_importation_document_page]	
AS
BEGIN
	DECLARE  @image varchar(500),@id_acte uniqueidentifier,@id_page1 uniqueidentifier=NULL,  @id_acte_xml varchar(255), @id_page uniqueidentifier, @id_fichier_xml int
			...
			,@id_zone_individu int, @id_acte_xml_comp varchar(255), @id_zone int, @id_zone_ithaque int, @id_zone_acte int,@id_zone_min int
	...
	SET @id_document = (SELECT id_document FROM Ithaque.dbo.AEL_Parametres WITH (NOLOCK) WHERE principal = 1)
	...
	CREATE TABLE [dbo].[temp_actexml_page_saisie]([id] [int] IDENTITY (1,1),[id_acte_ithaque] [int],[image] [varchar](255),[id_acte_xml] [varchar](255),[id_page] [uniqueidentifier]
			,[id_fichier_xml] [int] ,[id_fichier] [uniqueidentifier],[date_creation] [datetime],image_viewer [VARCHAR] (200))
