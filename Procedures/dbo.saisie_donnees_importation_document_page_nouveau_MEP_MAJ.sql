...
-- Description:	
-- =============================================
CREATE PROCEDURE [dbo].[saisie_donnees_importation_document_page_nouveau_MEP_MAJ]	
...
BEGIN
	DECLARE  @image varchar(500),@id_acte uniqueidentifier,@id_page1 uniqueidentifier=NULL,  @id_acte_xml varchar(255), @id_page uniqueidentifier, @id_fichier_xml int
			...
			,@id_zone_individu int, @id_acte_xml_comp varchar(255), @id_zone int, @id_zone_ithaque int, @id_zone_acte int,@id_zone_min int,@id_proiect uniqueidentifier,@id_association int
	...
	SELECT @id_document=id_document, @id_association=id_association FROM Ithaque.dbo.AEL_Parametres_MAJ_MEP WITH (NOLOCK) WHERE principal = 1
	SELECT @id_association=id_association FROM Ithaque.dbo.AEL_Parametres_nouveau WITH (NOLOCK) WHERE id_document=@id_document
...
---- actes		
	...
	BEGIN
		...
		INSERT INTO saisie_donnees_actes (type_acte ,date_acte ,annee_acte ,lieu_acte ,insee_acte 
			...
			,id_acte_ithaque,id_association)
...
-- cursor  -> test
	...
	INNER JOIN saisie_donnees_fichiers_XML sdf WITH (NOLOCK) ON taps.id_fichier_xml=sdf.id_fichier_xml
