...
GO
CREATE PROCEDURE [dbo].[finalize_importation]
--Test Run : [finalize_importation] 'F1DC2064-694D-48DC-924A-F6B039DA4634', 12802
...
BEGIN 
	...
	SET @id_projet = (SELECT id_projet FROM Ithaque.dbo.AEL_Parametres WHERE principal = 1 )
	...
	BEGIN TRANSACTION [finalize_importation];
		...
		WHERE a.id_document = @id_document AND b.id_document=@id_document
		...
		WHERE   ISNUMERIC(sda.annee_acte) = 1 AND ISNULL(id_statut_publication,0)=0
			AND sda.type_acte IN ( 0, 24, 134, 139, 133, 142, 143) 
		...
		AND (ISNULL(id_statut_publication,0)=0
			AND ((ISNUMERIC(sda.annee_acte) <> 1
					...
					OR type_acte NOT IN (0,24,134,139,2,105,136,138,141,1,3,4,5,6,7,8,10,13,17,21,135,137,140,143,133,142)
